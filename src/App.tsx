...
export default function App() {
  ...
  const isAnalyzed = analysisStep === 'complete' && hasValidCV && analyzedResumeText.length > 0;
  ...
  const hasPendingChanges = isAnalyzed && (stagedCount > 0 || analyzedResumeText !== resumeText);
  ...
  const handleSelectProfile = (profileId: string) => {
    ...
    setKeywords(profile.missingKeywords.map(k => ({ ...k, added: false, isStaged: false, isNewFlash: false })));
  ...
  const handleGenerateProfile = async (query: string) => {
    ...
    aiProgressTimers.current.push(
      setTimeout(() => {
        setLoadingProgressText(`Analysing industry requirements for "${role}"...`);
  ...
  // ─── Real-time resume keyword listener (only checks for words typed into resume) ───
  useEffect(() => {
    ...
    setKeywords((prevKeywords) => {
      ...
      const nextKeywords = prevKeywords.map((item) => {
        ...
        if (isPresentInText && !item.added) {
          updated = true;
          return { ...item, added: true, isStaged: false, isNewFlash: true };
        }
        return item;
      });
  ...
  useEffect(() => {
    ...
    if (hasFlashing) {
      const timer = setTimeout(() => {
        setKeywords(prev => prev.map(k => ({ ...k, isNewFlash: false })));
      }, 1500);
  ...
  const matchScore = useMemo(() => {
    if (!isAnalyzed || !hasValidCV || !currentProfile) return null;
    const baseline = currentProfile.matchScoreBaseline;
    const totalKeywords = keywords.length;
    if (totalKeywords === 0) return baseline;
    const addedCount = keywords.filter((k) => k.added || k.isStaged).length;
    const bonusPerKeyword = Math.round((100 - baseline) / totalKeywords);
    return Math.min(100, baseline + addedCount * bonusPerKeyword);
  }, [isAnalyzed, hasValidCV, currentProfile, keywords]);
  ...
  const handleStartAnalysis = (isUpdateRun: boolean = false) => {
    ...
    if (stagedKeywords.length > 0) {
      const bulletsToAdd = stagedKeywords
        .filter(k => !updatedResumeText.toLowerCase().includes(k.word.toLowerCase()))
        .map(k => `- Demonstrated proficiency in ${k.word} for key deliverables.`)
        .join('\n');
    ...
    setTimeout(() => {
      ...
      setLoadingProgressText('Extracting core missing keywords & calculating updated match score...');
      setLoadingProgressPercent(70);

      setTimeout(() => {
        setAnalysisStep('ats');
        setLoadingProgressText('Finalizing ATS gate verification & recommendations...');
        setLoadingProgressPercent(100);

        setTimeout(() => {
          // Mark staged keywords as added
          setKeywords(prev =>
            prev.map(k => (k.isStaged ? { ...k, added: true, isStaged: false } : k))
          );
  ...
  const handleAutoInsertKeyword = (word: string) => {
    const targetKw = keywords.find(k => k.word.toLowerCase() === word.toLowerCase());
    if (targetKw) {
  ...
  const handleSwapFluff = (oldWord: string, newMetric: string) => {
    const regex = new RegExp(oldWord.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, '\\$&'), 'gi');
    setResumeText((prev) => prev.replace(regex, newMetric));
  };

  const detectedCount = keywords.filter((k) => k.added || k.isStaged).length;
  ...
  return (
    <div className="min-h-screen bg-slate-100/70 text-slate-900 flex flex-col font-sans selection:bg-emerald-100 selection:text-emerald-900">
      ...
      <Header
        ...
        onReset={handleReset}
        isAnalyzing={analysisStep !== 'idle' && analysisStep !== 'complete'}
      />
      ...
      <main className="flex-1 max-w-[1700px] w-full mx-auto p-4 sm:p-6 lg:p-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-full items-start">
          ...
          {/* LEFT PANEL: Dual Text Inputs & Navy Action Button (5 cols) */}
          <div className="lg:col-span-5 h-full">
            <LeftPanel
              ...
              onAnalyze={() => handleStartAnalysis(isAnalyzed)}
              isAnalyzing={analysisStep !== 'idle' && analysisStep !== 'complete'}
          ...
          {/* RIGHT PANEL: Loading Bar / Interactive 3 Tabs (7 cols) */}
          <div className="lg:col-span-7 h-full">
            <RightPanel
              ...
              resumeText={resumeText}
              fluffDictionary={currentProfile?.fluffDictionary ?? []}
              starQuestions={currentProfile?.starQuestions ?? []}
              onAutoInsertKeyword={handleAutoInsertKeyword}
              onToggleStageKeyword={handleToggleStageKeyword}
              onSwapFluff={handleSwapFluff}
              onStartAnalysis={() => handleStartAnalysis(isAnalyzed)}
              isAiGenerated={currentProfile?.isAiGenerated}
              hasValidCV={hasValidCV}
              hasValidJob={hasValidJob}
      ...
      <footer className="bg-white border-t border-slate-200 py-3 text-center text-xs text-slate-500 font-medium">
        <div className="max-w-[1700px] mx-auto px-4 flex items-center justify-between">
          <span>Resumaxx Enterprise AI Application Coach © 2026</span>
          <span className="font-mono text-[11px] text-slate-400">Production SaaS Architecture • Modern UI</span>
        </div>
      </footer>
